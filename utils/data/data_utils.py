...
def create_prompt_dataset(
    ...
    # if cache_found:
    #     print(f"Prompt dataset cache {data_path} found in {output_path}")
    # else:
    #     print(f"Prompt dataset cache {data_path} not found in {output_path}")
    ...
    # print buf_create_cache
    # print(f"buf_create_cache: {buf_create_cache.item()}")
    ...
    if local_rank <= 0 and (buf_create_cache.item() != 0 or reload):
        ...
        if train_phase == 1 and len(sft_only_data_path)>0:
            sft_train_datasets = []
            sft_eval_datasets = []
            sft_train_size = 0
            sft_eval_size = 0
...
class DataCollatorRLHF:
    ...
    def __call__(self, data):
        ...
        if pad_length > 0:
            ...
            batch["attention_mask"] = F.pad(
                prompt_mask, pad=(0, pad_length), mode="constant", value=0
            )
        else:
            batch["input_ids"] = prompt
            batch["attention_mask"] = prompt_mask
        batch["input_ids"] = batch["input_ids"].flip(1)
        batch["attention_mask"] = batch["attention_mask"].flip(1)
        return batch
